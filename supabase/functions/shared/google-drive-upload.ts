...
export async function uploadStreaming(
  options: UploadOptions & { sourceUrl: string; timeoutMs?: number },
): Promise<string> {
  const timeoutMs = options.timeoutMs ?? 50_000;
  const uploadUrl = await buildResumableUploadUrl(options);

  const source = await fetch(options.sourceUrl, {
    redirect: "follow",
    signal: AbortSignal.timeout(timeoutMs),
  });
  if (!source.ok) {
    throw new Error(
      `Cloudinary fetch failed: HTTP ${source.status} for ${options.fileName}`,
    );
  }
  if (!source.body) {
    throw new Error("Cloudinary response has no body stream");
  }
...
export async function uploadChunked(
  options: ChunkUploadOptions & { sourceUrl: string; chunkCloseMs?: number },
